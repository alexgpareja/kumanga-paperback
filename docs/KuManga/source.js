...
var _Sources = (() => {
  ...
  // src/KuManga/KuManga.ts
  var KuManga_exports = {};
  __export(KuManga_exports, {
    KuManga: () => KuManga,
    KuMangaInfo: () => KuMangaInfo
  });
  ...
  var KuMangaInfo = {
    ...
    icon: "icon.png",
    author: "alexgpareja",
    description: "KuManga \u2014 Manga, Manhwa y Manhua en Espa\xF1ol",
    contentRating: import_types2.ContentRating.MATURE,
    websiteBaseURL: BASE_URL,
    language: "es",
    intents: 21
    // MANGA_CHAPTERS(1) | HOMEPAGE_SECTIONS(4) | CLOUDFLARE_BYPASS_REQUIRED(16)
  };
  function hexDecode(hex) {
    const bytes = hex.match(/.{2}/g) ?? [];
    return bytes.map((b) => String.fromCharCode(parseInt(b, 16))).join("");
  }
  function extractPageUrls($2) {
    const pages = [];
    const seen = /* @__PURE__ */ new Set();
    $2('img.lozad, img[data-src*="img.php"]').each((_, el) => {
    ...
    if (pages.length === 0) {
      const html3 = $2.html();
      for (const m of html3.matchAll(/img\.php\?src=([0-9A-Fa-f]{20,})/g)) {
  ...
  function buildMangaId(id, slug) {
    return `${id}_${slug}`;
  }
  function getNumericId(mangaId) {
  ...
  function parseStatus(t) {
    ...
    if (l.includes("activo") || l.includes("emisi\xF3n") || l.includes("ongoing")) return "Ongoing";
  ...
  function hasNextPage($2, currentPage) {
    const text3 = $2("body").text();
    const m = text3.match(/Mostrando\s+p[áa]gina\s+(\d+)\s+de\s+(\d+)/i);
    if (m) return parseInt(m[2]) > currentPage;
    return $2("a, button").toArray().some((el) => {
      const t = $2(el).text().trim().toLowerCase();
      return t === "siguiente" || t === "\xBB" || t === "next";
    });
  }
  function parseMangaTiles($2) {
    const tiles = [];
    const seen = /* @__PURE__ */ new Set();
    $2("li.km-li-crd").each((_, el) => {
      ...
      seen.add(mangaId);
      const imgEl = $2(el).find("img.km-img-crd").first();
      const image = imgEl.attr("src") || `https://static.kumanga.com/manga/6/${id}.jpg`;
      let title = imgEl.attr("alt") ?? "";
      if (!title) title = $2(el).find(".km-title-p-card").first().text().trim();
...
Please go to the homepage of <${BASE_URL}> and press the cloud icon.`);
    ...
    async getMangaDetails(mangaId) {
      ...
      const title = $2("h1").first().text().trim() || slug.replace(/-/g, " ");
      const image = $2('meta[property="og:image"]').attr("content") || `https://static.kumanga.com/manga/6/${numId}.jpg`;
      const desc = $2('meta[name="description"]').attr("content") || $2('meta[property="og:description"]').attr("content") || "";
      ...
      const tags = tagItems.length ? [App.createTagSection({ id: "genres", label: "G\xE9neros", tags: tagItems.slice(0, 20) })] : [];
    ...
    async getChapterDetails(mangaId, chapterId) {
      const numId = getNumericId(mangaId);
      const response = await this.requestManager.schedule(
        App.createRequest({ url: `${BASE_URL}/manga/${numId}/capitulo/${chapterId}`, method: "GET" }),
        1
      );
      this.checkResponseError(response);
      const $2 = load(response.data);
      const pages = extractPageUrls($2);
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const promises = [
        this.requestManager.schedule(App.createRequest({ url: `${BASE_URL}/mangalist?page=1`, method: "GET" }), 1).then((resp) => {
          this.checkResponseError(resp);
          const $2 = load(resp.data);
          const tiles = parseMangaTiles($2);
    ...
    async getViewMoreItems(sectionId, metadata) {
      ...
      return App.createPagedResults({
        results: parseMangaTiles($2),
        metadata: hasNextPage($2, page) ? { page: page + 1 } : void 0
      });
    }
    // ── getSearchResults ───────────────────────────────────────────────────
  ...
  return __toCommonJS(KuManga_exports);
})();
